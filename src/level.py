	...
	def setup_level(self,lvl):
		for row_index,row in enumerate(LEVEL_MAP[lvl]):
			for col_index,col in enumerate(row):
				...
				if col == 'Q':
					self.player2 = Player((x, y), [self.visible_sprites, self.active_sprites], [self.active_sprites, self.collision_sprites, self.fish_sprites, self.visible_sprites,self.door_sprites,self.igloo_sprites, self.limit_sprites],2, "Rose")
				if col == 'F':
